...
def main():
    ...
    log_event("=" * 60)
    log_event("MT5 XAU BOT START")
    log_event(f"Mode: {'DRY RUN' if dry_run else 'REAL TRADING'}")

    if not connect_mt5():
        log_event("MT5 connection failed")
        return

    if not ensure_symbol(symbol):
        log_event(f"Symbol check failed: {symbol}")
        return
    ...
    log_event(f"Last M15 candle: {last_processed}")
    log_event(f"Current M15 candle: {current_candle_time}")
    ...
    if current_candle_time == last_processed:
        log_event("M15 ALREADY PROCESSED")
        return
